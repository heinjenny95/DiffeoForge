"""Run the full block atlas optimizer on the committed five-subject CC0 cohort."""
...
from diffeoforge.engine import optimize_atlas
...
def run_smoke() -> dict:
    """Return deterministic full-atlas optimization evidence without writing files."""
    ...
    return {
        "schema_version": "0.4",
        "description": (
            "Deterministic full-parameter block optimization evidence on the committed "
            "five-subject CC0 synthetic cohort."
        ),
        "scientific_boundary": (
            "This is a dense CPU correctness prototype, not evidence of optimizer "
            "equivalence with Deformetrica, global convergence, topology preservation, "
            "or production readiness for 300 specimens."
        ),
        "runtime": {
        ...
        "result": {
            ...
            "parameter_sha256": {
                "template_vertices": _tensor_sha256(result.template_vertices),
                "control_points": _tensor_sha256(result.control_points),
                "momenta": _tensor_sha256(result.momenta),
            },
            "parameter_delta_norms": {
                "template_vertices": float(
                    torch.linalg.vector_norm(result.template_vertices - template)
                ),
                "control_points": float(
                    torch.linalg.vector_norm(result.control_points - control_points)
                ),
                "momenta": float(torch.linalg.vector_norm(result.momenta - initial_momenta)),
            },
            "history": history,
        },
    }
