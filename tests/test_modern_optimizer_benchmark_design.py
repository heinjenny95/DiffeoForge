...
def test_design_freezes_full_factorial_schedule_without_results() -> None:
    ...
    assert first["software"]["optimizer_benchmark_version"] == "0.2"
    assert first["software"]["engine_implementation"] == "1.7"
    assert first["configuration"]["momenta_updates_per_cycle"] == 1
