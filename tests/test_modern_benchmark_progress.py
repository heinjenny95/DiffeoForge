...
from diffeoforge.modern_benchmark_progress import (
    ...
    StudyProgressEvent,
    validate_study_progress_event,
)


def _condition(sequence: int = 1) -> StudyProgressCondition:
    return StudyProgressCondition(
        sequence=sequence,
        condition_id=f"condition-{sequence:03d}-subjects-00001-standard",
        pair_id="subjects-00001",
        subject_count=1,
...
def test_progress_events_are_immutable_strict_and_exact_count_based() -> None:
    ...
    condition_started = StudyProgressEvent(
        sequence=1,
    ...
    condition_completed = StudyProgressEvent(
        sequence=2,
    ...
    assert started.progress_version == "0.1"
    ...
    with pytest.raises(jsonschema.ValidationError):
        validate_study_progress_event(invalid)
...
def test_inconsistent_lifecycle_counts_are_rejected(
    status: str,
    completed: int,
    condition: StudyProgressCondition | None,
) -> None:
