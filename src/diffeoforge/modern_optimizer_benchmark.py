...
SCIENTIFIC_BOUNDARY = (
    "Each repeat uses a fresh process on the declared device and a deterministic prefix of the "
    "validated subjects. "
...
def _validate_report(report: dict[str, Any]) -> None:
    ...
    if declared_device is not None and environment_device != declared_device:
        raise ModernOptimizerBenchmarkError(
            "Optimizer benchmark device evidence is internally inconsistent"
        )
    cuda_sample_fields = {
    ...
    if declared_device == "cuda":
        ...
        if not required_environment <= report["environment"].keys() or any(
            ...
            raise ModernOptimizerBenchmarkError("CUDA optimizer telemetry is incomplete")
...
{momenta_schedule_html}{batch_workers_html}{device_html}
