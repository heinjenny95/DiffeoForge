...
SCIENTIFIC_BOUNDARY = (
    "Fresh processes on the declared device measure one explicitly declared exact objective plus "
    "gradient for the "
    "first configured "
    ...
    "complete allocator account, full-optimizer peak memory, end-to-end workflow runtime, an ETA "
    "model, an "
...
def _validate_report(report: dict[str, Any]) -> None:
    ...
    declared_device = report["configuration"].get("device")
    environment_device = report["environment"].get("device")
    if declared_device is not None and environment_device != declared_device:
        raise ModernBenchmarkError("Benchmark device evidence is internally inconsistent")
    cuda_sample_fields = {
    ...
    if declared_device == "cuda":
        ...
        if not required_environment <= report["environment"].keys() or any(
            not cuda_sample_fields <= sample.keys() for sample in samples
        ):
            raise ModernBenchmarkError("CUDA benchmark telemetry is incomplete")
...
def _prepare_problem(
    ...
    device = resolve_modern_torch_device(config["runtime"])
    ...
    count = control_config["count"]
    template = torch.tensor(vertices[0], dtype=torch.float64, device=device)
    triangles = torch.tensor(geometries[0].triangles, dtype=torch.int64, device=device)
    targets = tuple(
        (
            torch.tensor(subject_vertices, dtype=torch.float64, device=device),
            torch.tensor(geometry.triangles, dtype=torch.int64, device=device),
        )
    ...
    if control_config["method"] == "farthest_template_vertices":
        ...
        control_values = _read_control_point_rows(
            _resolve_from_config(control_config["path"], source),
            count,
        )
        controls = torch.tensor(control_values, dtype=torch.float64, device=device)
    momenta_config = config["initialization"]["momenta"]
    if momenta_config == "zeros":
        momenta = torch.zeros(
            (subject_count, controls.shape[0], 3),
            dtype=torch.float64,
            device=device,
        )
    else:
        all_momenta = _read_momenta_rows(
            _resolve_from_config(momenta_config["path"], source),
            tuple(path.name for path in inputs.subjects),
            count,
        )
        momenta = torch.tensor(
            all_momenta[:subject_count],
            dtype=torch.float64,
            device=device,
        )
...
def _measure_once(
    ...
    device = configure_modern_torch_runtime(config["runtime"])
    ...
    if device.type == "cuda":
        sample.update(
            {
                "cuda_memory_allocated_before_bytes": cuda_allocated_before,
                "cuda_memory_reserved_before_bytes": cuda_reserved_before,
    ...
    *,
    ...
    report = {
        ...
        "environment": {
            ...
            "device": config["runtime"]["device"],
            "deterministic_algorithms": config["runtime"]["device"] == "cuda",
            **(
                {
                    "cuda_runtime": torch.version.cuda,
                    "cuda_device_name": torch.cuda.get_device_name(0),
                    "cuda_device_total_memory_bytes": torch.cuda.get_device_properties(
                        0
                    ).total_memory,
...
def render_modern_benchmark_html(report: dict[str, Any]) -> str:
    ...
    device_html = (
        ...
        else f"\n<li>Execution device: {html.escape(config['device'])}</li>"
    )
    tile_rows = (
        "not applicable"
        if pairwise["mode"] == "dense"
...
<li>Threads: {config["threads"]}</li>{device_html}{implementation_html}
{pairwise_protocol_html}
