...
SCIENTIFIC_BOUNDARY = (
    "Fresh CPU processes measure one configured exact objective plus gradient for the first "
    "configured "
...
def _schema() -> dict[str, Any]:
    resource = files("diffeoforge.schema").joinpath("modern-benchmark-v0.2.json")
...
def _measure_once(
    ...
    for _ in range(warmup_evaluations):
        _objective_gradient(config, problem)
    gc.collect()
    ...
    try:
        objective, gradient_norm = _objective_gradient(config, problem)
    finally:
...
def _benchmark_worker(connection, config_path: str, subject_count: int, warmups: int) -> None:
    try:
        connection.send({"ok": True, "sample": _measure_once(config_path, subject_count, warmups)})
...
def _run_fresh_sample(
    ...
    process = context.Process(
        ...
        args=(sender, str(config_path), subject_count, warmup_evaluations),
...
def collect_modern_benchmark(
    ...
    *,
    ...
    for repeat in range(1, repeat_count + 1):
        sample = _run_fresh_sample(source, selected_count, warmups)
    ...
    report = {
        ...
        "benchmark_id": "configured_objective_gradient",
...
def _is_generated_report(directory: Path) -> bool:
    ...
    try:
        ...
        return False
    return report.get("benchmark_version") == BENCHMARK_VERSION and first_line == HTML_MARKER
    ...
    *,
    ...
    try:
        ...
        with (temporary / REPORT_CSV_NAME).open(
            ...
            for sample in report["samples"]:
                writer.writerow(sample[name] for name in columns)
