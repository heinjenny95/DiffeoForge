"""Resumable execution of frozen multi-tile benchmark matrix designs."""
...
from diffeoforge.modern_benchmark import (
    ...
    REPORT_HTML_NAME,
    REPORT_JSON_NAME,
    TILE_OVERRIDE_BENCHMARK_VERSION,
...
from diffeoforge.modern_benchmark_matrix_design import (
    MATRIX_DESIGN_JSON_NAME as DESIGN_JSON_NAME,
)
from diffeoforge.modern_benchmark_matrix_design import (
    collect_modern_benchmark_matrix_design as collect_modern_benchmark_design,
)
from diffeoforge.modern_benchmark_matrix_design import (
    verify_modern_benchmark_matrix_design as verify_modern_benchmark_design,
)
from diffeoforge.modern_benchmark_matrix_progress import (
    MatrixStudyProgressCallback as StudyProgressCallback,
)
from diffeoforge.modern_benchmark_matrix_progress import (
    MatrixStudyProgressCondition as StudyProgressCondition,
)
from diffeoforge.modern_benchmark_matrix_progress import (
    MatrixStudyProgressEvent as StudyProgressEvent,
)
from diffeoforge.modern_benchmark_matrix_progress import (
    MatrixStudyProgressObserverError as StudyProgressObserverError,
)
from diffeoforge.modern_benchmark_matrix_progress import (
    MatrixStudyProgressStatus as StudyProgressStatus,
)

STUDY_RUN_VERSION = "0.2"
...
SCIENTIFIC_BOUNDARY = (
    "This run preserves separate raw v0.4 objective/gradient benchmark reports in a frozen "
    "full-factorial matrix order. It performs no automatic comparison, winner selection, "
    "speedup or significance calculation, safe-preset selection, peak-memory inference, "
    "full-workflow measurement, scaling extrapolation, or 300-subject feasibility test."
)
...
def _manifest_schema() -> dict[str, Any]:
    resource = files("diffeoforge.schema").joinpath("modern-benchmark-study-run-v0.2.json")
...
def _verify_condition_report(
    ...
    expected_configuration = {
        ...
        "source_pairwise_evaluation": frozen["source_pairwise_evaluation"],
        ...
        condition_records.append(
            {
                ...
                "effective_pairwise_evaluation": condition[
...
def verify_modern_benchmark_matrix_study_run(directory: Path | str) -> dict[str, Any]:
    """Verify a complete matrix study run and every separate raw v0.4 report."""
...
def inspect_modern_benchmark_matrix_study_run(directory: Path | str) -> dict[str, Any]:
    """Strictly inspect partial or complete matrix evidence without changing it."""
    ...
    if state["status"] == "complete":
        verify_modern_benchmark_matrix_study_run(root)
    ...
    return {
        ...
        "next_condition": (
            ...
            if next_condition is None
                ...
                "effective_pairwise_evaluation": next_condition[
                    "effective_pairwise_evaluation"
                ],
...
def default_modern_benchmark_matrix_study_path(design_directory: Path | str) -> Path:
    root = Path(design_directory).expanduser().resolve()
    return root.parent / f"{root.name}.run"


def run_modern_benchmark_matrix_study(
    ...
    *,
    ...
    output = (
        default_modern_benchmark_matrix_study_path(design_root)
    ...
    with _study_lock(output):
        ...
        if state["status"] == "complete":
            ...
            verify_modern_benchmark_matrix_study_run(output)
            return output
        ...
        for condition in design["conditions"][len(existing_ids) :]:
            ...
            try:
                ...
                benchmark_modern_objective(
                    ...
                    tile_autograd_strategy=condition["tile_autograd_strategy"],
                    query_tile_size=condition["effective_pairwise_evaluation"][
                        "query_tile_size"
                    ],
                    source_tile_size=condition["effective_pairwise_evaluation"][
                        "source_tile_size"
                    ],
