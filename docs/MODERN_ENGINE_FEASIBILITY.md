...
- Euler and Heun template-point flow;
...
objective assembly across subjects, optimization, output meshes, checkpointing,
GPU execution, sparse/chunked kernels, or a workflow-backend adapter.
...
1. Add current/varifold fixtures generated through the independent reference
   environment rather than only invariant tests.
